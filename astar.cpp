...
void  pokoloruj_droge(node *Q, char map[][N])
{
    ...
    while(akt != NULL)
    {
        if(map[akt->gety()][akt->getx()] != 'S' && map[akt->gety()][akt->getx()] != 'M')
            map[akt->gety()][akt->getx()] = 'D';
        akt = akt->getr();
    }
}
...
node * algorytm_a_gwiazdka(int xstart, int ystart, int xcelu, int ycelu, char map[][N])
{
    ...
    while(otwarta_glowa != NULL)
    {
        ...
        for(int i = 0; i < 8; i++)
        {
            ...
            if(!czy_jest_w_liscie(zamknieta_glowa, x_s, y_s) && !czy_zakazane(map, x_s, y_s))
            {

                if(!czy_jest_w_liscie(otwarta_glowa, x_s, y_s))
                {
                       dodaj_do_otwartej(otwarta_glowa, x_s, y_s, xcelu, ycelu, Q, map, zaznaczanie);
                }
                else
                {
                    tmp = (Q->getr());
